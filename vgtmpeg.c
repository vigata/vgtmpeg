...
void av_noreturn exit_program(int ret)
{
    ...
    if( ret>0 ) {
        ...
        for(i=0;i<nb_output_files;i++) {
            if( output_files[i].wrote_header ) {
                         ...
                         int is_last_report, int64_t timer_start, int64_t cur_time)
{
    ...
    if( output_xml )
        print_nlreport( output_files, ost_table, nb_ostreams, is_last_report, timer_start );
                          ...
                          InputFile  *input_files,  int nb_input_files)
{
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        output_files[i].wrote_header = 1; /* flagging wrote header. --vgtmpeg */
...
//        fprintf(stderr, "next:%"PRId64" dts:%"PRId64" off:%"PRId64" %d\n", ist->next_pts, pkt.dts, input_files[ist->file_index].ts_offset, ist->st->codec->codec_type);
        if (pkt.dts != AV_NOPTS_VALUE && ist->next_pts != AV_NOPTS_VALUE
            ...
            if((delta < -1LL*dts_delta_threshold*AV_TIME_BASE ||
                ...
                /* || pkt_dts+1<ist->pts  This causes audio gaps in some test samples. --vgtmpeg */)&& !copy_ts){
