...
//        fprintf(stderr, "next:%"PRId64" dts:%"PRId64" off:%"PRId64" %d\n", ist->next_pts, pkt.dts, input_files_ts_offset[ist->file_index], ist->st->codec->codec_type);
        if (pkt.dts != AV_NOPTS_VALUE && ist->next_pts != AV_NOPTS_VALUE
            ...
            if((FFABS(delta) > 1LL*dts_delta_threshold*AV_TIME_BASE /* vgmpeg|| pkt_dts+1<ist->pts*/ )&& !copy_ts){
...
static int64_t getmaxrss(void)
{
    ...
    struct rusage rusage;
    getrusage(RUSAGE_SELF, &rusage);
    //return (int64_t)rusage.ru_maxrss * 1024; FIXME. this is not available on macos?
...
static void show_usage(void)
{
    ...
    printf("usage: vgtmpeg [options] [[infile options] -i infile]... {[outfile options] outfile}...\n");
    printf("\n");
}
...
#include "vgtmpeg_opts.h"
    ...
    { "v", HAS_ARG, {(void*)opt_verbose}, "set vgtmpeg verbosity level", "number" },
...
int main(int argc, char **argv)
{
    ...
    if(nb_output_files <= 0 && nb_input_files == 0) {
        show_usage();
        fprintf(stderr, "Use -h to get full help or, even better, run 'man vgtmpeg'\n");
        vgtmpeg_exit(1);
    }
